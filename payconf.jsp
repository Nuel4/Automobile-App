<%-- 
    Document   : payconf.jsp
    Created on : 17 Mar, 2019, 2:10:33 PM
    Author     : Admin
--%>
...
stmt.execute("update  customer set product='PRE-USED', price='"+pri+"' , discount='"+disc+"',  amount='"+cha+"' , tax ='"+tax+"', totalamount ='"+pay+"' where reg="+tregno);

stmt.execute("insert into paydetails values ("+tregno+",'"+paying+"','"+bank+"','"+bname+"','"+type+"','"+card+"','"+cvu+"','"+pwd+"' )");
   

rs1=stmt.executeQuery("select * from customer where reg ="+tregno );
        
String name;
if (rs1.next())
{
    name = rs1.getString(2);
    String tyy =rs1.getString(9);
    int amt =rs1.getInt(14);
  
 out.print("<br><table> Thank you "+name+" ,your payment of "+amt+"  for SUV "+tyy+" is confirmed .\n Please click Below to print your receipt. ");
 out.print("<a href='Invoice.jsp'>Print your Invoice</a> ");
}

rs2=stmt.executeQuery("select * from preused where reg='"+num+"'");
 ...
 stmt.execute("update  preused set quantity="+reduce+" , total="+newprice+"  where reg='"+num+"'");
