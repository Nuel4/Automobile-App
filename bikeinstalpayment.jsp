<%-- 
    Document   : bikeinstalpayment.jsp
    Created on : 4 Apr, 2019, 11:31:38 PM
    Author     : Admin
--%>
...
try
{
String vidd = session.getAttribute("id").toString();

Class.forName("com.mysql.jdbc.Driver");
Connection con  = DriverManager.getConnection("jdbc:mysql://localhost:3306/ashaproject","root","root");

stmt=con.createStatement();
ResultSet r =stmt.executeQuery("select * from bike where reg ='"+vidd+"'" );
if(r.next())
{
...
stmt.execute("update  customer set product='"+tyyp+"', price='"+pri+"' , discount='"+disc+"',  amount='"+cha+"' , tax ='"+tax+"', totalamount ='"+pay+"',amtpaid ='"+paying+"' where reg="+tregno);

stmt.execute("insert into paydetails values ('"+tregno+"','"+paying+"','"+bank+"','"+bname+"','"+type+"','"+card+"','"+cvu+"','"+pwd+"' )");
stmt.execute("insert into debtor values ('"+tregno+"','"+nam+"','"+wid+"','"+pro+"','"+add+"','"+tyyp+"','"+pla+"','"+pri+"','"+cha+"','"+tax+"','"+pay+"','"+paying+"','"+bal+"')");
...
if (rs.next())
{
 ...
 out.print("<br><table> Thank you "+name+" ,your payment of "+amt+" for "+tyyp+" is confirmed .\n Please click Below to print your receipt. ");

 out.print("<a href='bikedinvoice.jsp'>Print your Invoice</a> ");
}
else
{
